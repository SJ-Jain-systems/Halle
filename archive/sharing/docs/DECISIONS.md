...
The three decisions the brief left open, and what I did about each.

## 1. Where the data comes from

I use PLOS's search engine (Solr) to find the psychology papers, and the
downloaded article files (allofplos) only for the full text. Search engine for
discovery, local files for reading.

I did not start there. The first plan was to skip the search engine entirely:
download every PLOS article and read the subject tags straight out of the files.
That failed for a reason worth knowing.

When I counted psychology papers per year, 2015 came back with 12 papers, versus
...
The search engine does have the tags for every article. So I switched: use it to
...
described, before I tried the shortcut.

Two smaller things I hit on the way:
...
- The brief named five subfields, but PLOS has no "Quantitative psychology" tag
  and uses several the brief didn't list. So I take whatever subfield PLOS
  actually assigns, which gives 23 subfields instead of 5.

## 2. The pilot sample

Before running the model on all 58,000 papers, I test it on a small batch and
grade it by hand.

I draw 2 papers from each subfield, not 46 at random. A random draw would be
