...
Usage (on a Rivanna GPU node — see docs/RUNNING_ON_RIVANNA.md):
    python -m src.compare_llms --sample data/sampled_articles.csv --backend vllm
...
def main() -> None:
    ...
    parser.add_argument(
        "--model",
        default=None,
        help="Run only this one model instead of every candidate_models entry in config.yaml. "
        "Recommended on HPC: submit one SLURM job per model rather than loading both "
        "70B and 7B models sequentially in the same GPU process.",
    )
    args = parser.parse_args()

    with open(args.config, encoding="utf-8") as f:
        config = yaml.safe_load(f)
    models = [args.model] if args.model else config["candidate_models"]

    def client_factory(model_id: str):
        kwargs = {"tensor_parallel_size": args.tensor_parallel_size} if args.backend == "vllm" else {}
        return build_client(model_id, backend=args.backend, **kwargs)
