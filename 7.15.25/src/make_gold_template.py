...
produces (the keys in src/extract_demographics.py, REQUIRED_KEYS), one row per
...
number); if there's a number, put it in ses_value.
...
def dump_texts(articles: list[dict], text_dir: str) -> int:
    """Write each article's methods text so you read what the model reads. Returns
